...
An HTTP proxy that downloads the M3U channel playlist from a [TvHeadend](https://tvheadend.org/) server and rewrites each stream URL to embed HTTP basic-auth credentials inline.

TvHeadend's default playlist uses per-channel `?ticket=...` tokens that expire after 300 seconds, which breaks downstream consumers like IPTV managers and DVR front-ends. This proxy rewrites each URL to carry inline credentials instead of an expiring ticket, so consumers keep a working channel list:
...
The proxy emits a standard M3U playlist, so it should work with any client that reads an M3U URL. The names below are examples rather than a tested-compatibility matrix; none have been individually verified against this proxy.

- xTeVe, the original target for this proxy, is no longer maintained.
- [Threadfin](https://github.com/Threadfin/Threadfin) is the actively maintained fork of xTeVe and the recommended successor.
- StreamMaster and IPTVBoss are other M3U playlist managers.
- Plex DVR, Jellyfin, and Emby can read an M3U tuner directly.

Point any of them at `http://<proxy-host>:33721/api/tvheadend/channels` to ingest the channel list.
...
|---|---|---|---|
| `TVHEADEND__ADDRESS` | yes | (required) | TvHeadend base URL including scheme and port (e.g. `http://192.168.1.10:9981`) |
| `TVHEADEND__USERNAME` | yes | (required) | TvHeadend HTTP basic-auth username |
| `TVHEADEND__PASSWORD` | yes | (required) | TvHeadend HTTP basic-auth password |
...
> - The optional `PROXY_API_KEY` gates the `/api/tvheadend/channels` endpoint but does not encrypt the stream URLs or the channel list in transit.
...
To validate a release candidate without pushing, trigger `workflow_dispatch` with `dry_run: true`. This builds the image without pushing it.
